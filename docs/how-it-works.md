...
5. **Pin-check.** `repocodex validate --diff`. The required check attests that anchors, claims, the skipped-memory ratchet, and the reverse index still match live text.
...
Skipping retrieval does not make the change invisible. Hook and CI still fail later on `CLAIM_BROKEN`, unrepaired `DRIFT`, or skipped-memory on a covered file.
