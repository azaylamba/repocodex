...
The system SHALL emit REANCHOR patches carrying every field the relocation changed — the pinned `path` and any relocated terms — together with an `actor` of `process:repocodex-reanchor`, so that a caller applying the patch verbatim produces a concept that attests. A successful reanchor SHALL NOT replace or append `verified` by itself; that field records definition review, not a pin receipt.
