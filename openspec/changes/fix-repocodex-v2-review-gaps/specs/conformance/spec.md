...
**Reason**: This bound “no existing scenario is broken” to a pytest mapping. The product’s verification is not a test suite. The why of a change or implementation lives in OKF; agents retrieve those concepts, read the code they pin, and judge whether the scenario still holds. A table of test names is a second, weaker system, and it rotted inside the change that created it.

**Migration**: Replaced by “Why is stored as linked OKF and served to agents” and “Scenario integrity is judged by reading why and code”. Delete `src/repocodex/conformance.py` and its scenario-to-test table.

### Requirement: Scenario tests assert single expected outcomes

**Reason**: Scoped to the same pytest mapping. Scenario outcomes are judged by the agent against the retrieved concepts and the live code, not asserted in a test.

**Migration**: Replaced by “Scenario integrity is judged by reading why and code”. Engine-package tests that pin the CLI and the attester are out of this capability; they are not how application scenarios are verified.
